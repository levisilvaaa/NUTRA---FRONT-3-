...
interface ThreeBottleDownsell2OfferPopupProps {
  isOpen: boolean;
  onClose: () => void;
  sixBottleUrl: string;
  oneBottleUrl: string;
...
const ThreeBottleDownsell2OfferPopup: React.FC<ThreeBottleDownsell2OfferPopupProps> = ({
  isOpen,
  onClose,
  sixBottleUrl,
  oneBottleUrl,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-20 p-2 sm:p-4 min-h-screen w-full">
      ...
      {/* Popup Content */}
      <div className="relative bg-gradient-to-br from-gray-900 via-black to-gray-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl border-2 sm:border-4 border-gray-700/50 max-w-sm sm:max-w-md w-full mx-2 sm:mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-4 sm:mb-6">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            <span className="text-red-400 font-black text-base sm:text-xl">
        ...
        <div className="text-center mb-4 sm:mb-6">
          ...
          {/* Offer Details */}
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-3 sm:mb-4 border border-gray-600/30">
            <h3 className="text-lg sm:text-xl font-black text-red-400 mb-2 sm:mb-3 leading-tight">
        ...
        <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-black text-lg sm:text-xl py-3 sm:py-4 rounded-xl sm:rounded-2xl shadow-xl transform hover:scale-105 transition-all duration-200 border-2 border-red-400/30 relative overflow-hidden"
          >
            {/* Button shine effect */}
          ...
          <button
            ...
            className="w-full bg-transparent border-2 border-gray-600 hover:border-gray-500 text-gray-300 hover:text-white font-bold text-base sm:text-lg py-2.5 sm:py-3 rounded-xl sm:rounded-2xl transition-all duration-200 hover:bg-gray-800/50"
          >
            Refuse Offer
          </button>
        </div>
        ...
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          <div className="absolute top-4 sm:top-8 left-2 sm:left-3 w-8 sm:w-12 h-8 sm:h-12 bg-red-500/20 rounded-full blur-xl"></div>
          <div className="absolute bottom-4 sm:bottom-8 right-2 sm:right-3 w-12 sm:w-16 h-12 sm:h-16 bg-gray-700/30 rounded-full blur-xl"></div>
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 sm:w-32 h-24 sm:h-32 bg-gray-800/20 rounded-full blur-3xl"></div>
        </div>

        {/* Timer Progress Bar */}
        <div className="absolute bottom-0 left-0 w-full h-1.5 sm:h-2 bg-gray-800/50 rounded-b-2xl sm:rounded-b-3xl overflow-hidden">
          <div 
            className="h-full bg-gradient-to-r from-red-500 to-red-600 transition-all duration-1000 ease-linear relative"
            style={{ width: `${(timeLeft / 20) * 100}%` }}
          >
            {/* Progress bar glow effect */}
            <div className="absolute inset-0 bg-gradient-to-r from-red-400 to-red-500 blur-sm opacity-50"></div>
          </div>
        </div>
      </div>
    </div>
...
export default ThreeBottleDownsell2OfferPopup;